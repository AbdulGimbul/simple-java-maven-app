            ...
            steps {
                ...
                script {
                    ...
                    sshagent(credentials: ['jenkins-to-aws']) {
                        ...
                        sh "ssh -o StrictHostKeyChecking=no -i ./jenkins/scripts/abdl_aws_key.pem app@${ec2PublicIp} 'if pgrep -f *.jar; then pkill -f *.jar; fi && cd ${remoteDir} && java -jar ${jarFileName} &'"
                    }
                }
            }
        }
